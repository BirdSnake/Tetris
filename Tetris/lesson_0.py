    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == KEYUP:
                ...
                """Обработка событий нажатия на клавишу"""
                ...
                # Поворачиваем фигуру, если есть место
                    ...
                    for i in range(1, CUP_HEIGHT):
                        if not check_pos(cup, falling_fig, adj_y=i):
                            break
                    falling_fig['y'] += i - 1
        """Управление падением фигуры при удержаний клавиш"""
...
def game_cup(cup):
    ...
    pygame.draw.rect(DISPLAY_SURF, BORDER_COLOR, (SIDE_MARGIN - 4, TOP_MARGIN - 4, (CUP_WIDTH * BLOCK) + 8, (CUP_HEIGHT * BLOCK) + 8), 5)
...
def draw_title():
    ...
    title_rect.topleft = (WINDOW_WIDTH - 400, 30)

def clear_completed(cup):
    """Удаление зполненных рядов и сдвиг верхних рядов вниз"""
    removed_lines = 0
    y = CUP_HEIGHT - 1
    while y >= 0:
        if is_competed(cup, y):
            for push_down_y in range(y, 0, -1):
                for x in range(CUP_WIDTH):
                    cup[x][0][push_down_y] = cup[x][push_down_y - 1]

def is_competed(cup, y):
    """Проверка полностью заполненных рядов"""
    for x in range (CUP_WIDTH):
        if cup[x][y] == EMPTY:
            return False
    return True
...
def check_pos(cup, fig, adj_x=0, adj_y=0):
    """Проверка возможности создания фигуры в стакане"""
    for x in range(FIGURE_WIDTH):
        for y in range(FIGURE_HEIGHT):
...
def get_new_fig():
    """Создает новую фигуру со случайным цветом и углом поворота"""
